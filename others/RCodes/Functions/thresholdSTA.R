...
thresholdSTA <- function(img, k, iter=10, maxRotationFactor=1, minRotationFactor=0.0001, 
  ...
  for (i in 1:searchEnforcement){
    for (j in 1:(k-1)){
      solutions[i,j] <- round(minThr + runif(1)*(maxThr-minThr)) # inicializa cada celula con un valor aleatorio entre 0 y L-1
    }
  }
  ...
  counter <- 0
  rotationFactor <- runif(1) #random(0,1) uniforme
  ...
  repeat{
    ...
    for (i in 1:searchEnforcement){
      ...
      # si alguno se fue de rango tras transformar, revertir el estado
      ...
      # si hay K umbrales y el primero no es 0 y el último no es el max
