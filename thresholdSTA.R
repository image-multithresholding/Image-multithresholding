...
thresholdSTA <- function(img, k, iter=1500, maxRotationFactor=1, minRotationFactor=0.0001, 
  ...
  repeat{
    ...
    for (i in 1:searchEnforcement){
      ...
      oldStateError <- errorSTA(prob, oldState[1:k], oldState[(k+1):(2*k)], oldState[(2*k+1):(3*k)], penalty)
      stateError <- errorSTA(prob, state[1:k], state[(k+1):(2*k)], state[(2*k+1):(3*k)], penalty)
      ...
      if(is.na(stateError) == FALSE & stateError < oldStateError){
        ...
        newStateError <- errorSTA(prob, newState[1:k], newState[(k+1):(2*k)], newState[(2*k+1):(3*k)], penalty)
  ...
  for (i in 1:searchEnforcement){
    fittingError[i] <- errorSTA(prob, solutions[i,1:k], solutions[i,(k+1):(2*k)], solutions[i,(2*k+1):(3*k)], penalty)
  }
