...
def threshold_pso(img: np.ndarray, k: int, options: PSOOptions, iterations: int = 10):
    ...
    particle_position = np.array([[random.randint(0, L-1) for _ in range(k)] for _ in range(options.number_part)])
    particle_position = np.sort(particle_position)

    particle_fitness = [0 if not is_valid_position(v, k, L) else _between_class_var(prob, v) for v in particle_position]
    ...
    counter = 0
    particle_velocity = np.array([[0 for _ in range(k)] for _ in range(options.number_part)])

    while counter < iterations:
        inertia = (options.inertia_initial - options.inertia_final) * (iterations-counter-1)/iterations + options.inertia_final
        acceleration_1 = options.acceleration_1_initial + (options.acceleration_1_final - options.acceleration_1_initial) * (counter+1)/iterations
        acceleration_2 = options.acceleration_2_initial + (options.acceleration_2_final - options.acceleration_2_initial) * (counter+1)/iterations
        ...
        for i in range(options.number_part):
            ...
            particle_velocity[i] = inertia * particle_velocity[i] + \
                ...
                acceleration_2 * r2 * (global_best_position - particle_position[i])
