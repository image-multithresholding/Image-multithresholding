...
import random
import math
from PythonCodes.library.base import _between_class_var, _cluster_mean, _cluster_var, _image_histogram, _image_probabilities, _prob_up_to_level, _gray_clustering
...
def _within_class_var(prob: List[float], levels: List[int]) -> int:
    ...
    total_mean = _cluster_mean(prob, [i for i, _ in enumerate(prob)], 0, adjust=0)
...
def _binary_to_decimal(chromo: List[Literal[0, 1]], K: int) -> List[int]:
    ...
    for x in range(K):
        ...
        for off in range(number_length):
            current_value += chromo[x*number_length + off]*2**(number_length-off-1)
...
def threshold_gaa(img: np.ndarray, k: int, iter: int=200, pop_size: int=10, crossover_rate: float=0.95, mutation_rate: float=0.05):
    ...
    for j in range(iter):
        ...
        while new_size < pop_size:
            ...
            if r < mutation_rate:
                new_chr_a, new_chr_b = _mutation(new_chr_a), _mutation(new_chr_b)
        ...
        fit_new_pop = [_fitness_function(prob, _binary_to_decimal(p, k)) for p in new_pop]
    ...
    fit_best = [_fitness_function(prob, _binary_to_decimal(p, k)) for p in best_hip]
