...
def _rotation(x: np.array, rotationFactor: float):
    n = len(x)
    mat = np.matrix([[random.random()*2-1 for _ in range(n)] for _ in range(n)])
...
def _transform_and_translate(prob: List[float], k: int, transform_fun: Callable, state: np.array, fun_factor: float, translation_factor: float, min: int, max: int):
    new_solution, has_better_fit = _transform(prob, k, transform_fun, state, fun_factor, min, max)
    if has_better_fit:
...
def threshold_sta(img: np.ndarray, k: int, iter: int=10, max_rotation_factor: float=1,
    ...
    for i in range(search_enforcement):
        for j in range(k):
            ...
            solutions[i][j] = round(min_thr + random.random()*(max_thr-min_thr))
    ...
    while counter <= iter:
        ...
        for i in range(search_enforcement):
            solutions[i] = _transform_and_translate(prob, k, _expansion, solutions[i], expansion_factor, translation_factor, min_thr, max_thr)
        for i in range(search_enforcement):
            solutions[i] = _transform_and_translate(prob, k, _rotation, solutions[i], rotation_factor, translation_factor, min_thr, max_thr)
        for i in range(search_enforcement):
            solutions[i] = _transform_and_translate(prob, k, _axesion, solutions[i], axesion_factor, translation_factor, min_thr, max_thr)
