...
def GaussianError(penalty: float = 1.5):
    class GE:
        ...
        def calculate_best_solution(self):
            self.fitting_error = [
                _objective_error(self.prob, self.prior_prob[i], self.mean[i], self.var[i], self.penalty)
        ...
        def crossover(self, crossover_prob):
            self.crossed_vectors_prior_prob = np.ndarray(shape=(self.number_pop, self.k+1), dtype=float)
            self.crossed_vectors_mean = np.ndarray(shape=(self.number_pop, self.k+1), dtype=int)
            self.crossed_vectors_var = np.ndarray(shape=(self.number_pop, self.k+1), dtype=int)
            ...
            for i, _ in enumerate(self.mutant_vectors_prior_prob):
                if (random.random() < crossover_prob) and self._is_solution_in_range(self.mutant_vectors_prior_prob[i], self.mutant_vectors_mean[i], self.mutant_vectors_var[i]):
                    ...
                    if self.crossed_vectors_var[i][0] < 0:
                        print(self._is_solution_in_range(self.mutant_vectors_prior_prob[i], self.mutant_vectors_mean[i], self.mutant_vectors_var[i]))
                else:
        ...
        def new_fitting_error(self):
            self.nfe = [_objective_error(self.prob, self.crossed_vectors_prior_prob[i], self.crossed_vectors_mean[i], self.crossed_vectors_var[i], self.penalty) for i, _ in enumerate(self.crossed_vectors_prior_prob)]
        ...
        def get_result(self) -> List[int]:
            ...
            B = [2*(mean[i]*var[i+1]**2-mean[i+1]*v**2) for i, v in enumerate(var[:-1])]
            C = [(v*mean[i+1])**2-(var[i+1]*mean[i])**2+2*(v*var[i+1])**2*np.log((var[i+1]*prior_prob[i])/(v*prior_prob[i+1])) for i, v in enumerate(var[:-1])]
